...
local compileCommands = function(opts)
    ...
    pickers.new(opts, {
        ...
        finder = finders.new_oneshot_job({"bash", "-c", "cd $WS_ROOT/build && find -name compile_commands.json -printf '%h\n'"}, opts),
        ...
        attach_mappings = function(prompt_bufnr, map)
            actions.select_default:replace(function()
                ...
                vim.g.clangd_compile_commands_dir = "${WS_ROOT}/build/" .. selection[1]
                vim.cmd("LspRestart")
            end)
            return true
        end
